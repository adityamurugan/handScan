...
with mp_hands.Hands(
    ...
    for files in filesPath:
      ...
      os.makedirs('handScanResults/annotated/' + folderName, exist_ok=True) 
      os.makedirs('handScanResults/Coords/IC_Hands/' + folderName, exist_ok=True) 
      ...
      cv2.imwrite('handScanResults/annotated/' + folderName + '/annotated_image' + fname, image)
      cv2.waitKey(1)
      row.insert(0,fname)
      rows.append(row)
      # print(len(rows))
      ...
      df.columns = col
      df.to_csv('handScanResults/Coords/IC_Hands/' + folderName + '/' + folderName + '_coords_IC.csv')